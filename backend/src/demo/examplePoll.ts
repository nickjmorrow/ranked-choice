 ...
 * already cast so its results have something to show. Used by the migration
 * that first creates it and by the nightly demo reset that restores it, so the
 ...
 *
 * Changing anything here changes what a freshly migrated database contains,
 * and what the demo is restored to — never an existing database's history.
 */

/** Anything that can run SQL: a QueryRunner in a migration, an EntityManager elsewhere. */
export interface Sql {
  query(sql: string, parameters?: unknown[]): Promise<unknown>;
}

export const EXAMPLE_LINK = 'example';
...
/** Creates the example poll and casts its ballots. */
export async function seedExamplePoll(queryRunner: Sql): Promise<void> {
...
/** Deletes a poll and everything under it. No foreign key cascades, so bottom up. */
export async function deletePoll(queryRunner: Sql, link: string) {
